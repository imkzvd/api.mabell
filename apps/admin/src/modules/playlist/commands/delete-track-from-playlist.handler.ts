...
{
  private readonly _coreHandler: CoreDeleteTrackFromPlaylistHandler;

  constructor(
    @Inject(PlaylistService) readonly playlistService: PlaylistService,
    @Inject(TrackService) readonly trackService: TrackService,
  ) {
    this._coreHandler = new CoreDeleteTrackFromPlaylistHandler(playlistService, trackService);
  }

  execute(command: DeleteTrackFromPlaylistCommand) {
